...
let stations=Array.from({length:28},(_,index)=>{
  ...
  const totalChange=index%4===0?0:index%4===1?1250:index%4===2?-620:380;
...
function applyFilters(){
  const year=document.getElementById('yearFilter').value,code=document.getElementById('codeFilter').value.trim().toLowerCase(),name=document.getElementById('nameFilter').value.trim().toLowerCase(),status=document.getElementById('statusFilter').value,type=document.getElementById('typeFilter').value;
...
function resetFilters(){
  document.getElementById('yearFilter').value=CURRENT_YEAR;['codeFilter','nameFilter','statusFilter','typeFilter','yearStart','yearEnd','dateStart','dateEnd'].forEach(id=>document.getElementById(id).value='');selectedIds.clear();currentPage=1;applyFilters();renderTree();renderTable();
}
...
function formatArea(value){return Number(value).toLocaleString('zh-CN',{minimumFractionDigits:2,maximumFractionDigits:2});}
